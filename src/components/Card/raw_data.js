export default [
        {
          color: 'FEC006',
          title: 'Snow in Turkey Brings Travel Woes',
          thumbnail: '',
          category: 'News',
          excerpt: 'Heavy snowstorm in Turkey creates havoc as hundreds of villages left without power, and hundreds of roads closed',
          date: new Date()
        },
        {
          color: '2196F3',
          title: 'Landslide Leaving Thousands Homeless',
          thumbnail: '',
          category: 'News',
          excerpt: 'An abort landslide in the Silicon Valley has left thousands homeless and on the streets.',
          date: new Date()
        },
        {
          color: "FE5621",
          title: "Hail the size of baseballs in New York",
          thumbnail: "",
          category: "News",
          excerpt: "A rare and unexpected event occurred today as hail the size of snowball hits New York citizens.",
          date: new Date()
        },
        {
          color: "673AB7",
          title: "Earthquake destroying San Francisco",
          thumbnail: "",
          category: "News",
          excerpt: "A massive earthquake just hit San Francisco leaving behind a giant crater.",
          date: new Date()
        }
    ];